...
// POSIX isatty(3) probe: tcgetattr (TIOCGETA on darwin) succeeds only
// on a terminal. A ModeCharDevice check is NOT sufficient: /dev/null
// is a character device, so `doctor --fix </dev/null` would wrongly
